...
func getLogicalRouters(ovnPod string, chassisDict Chassis.ChassisDict) []logicalRouter {
	tools := Tools.New()
	kubeCtlCmd := exec.Command("sudo", "/usr/bin/kubectl", "exec", ovnPod, "ovn-nbctl", "list", "Logical_Router")
...
func getLogicalRouterPorts(ovnPod string, chassisDict Chassis.ChassisDict) []logicalRouterPort {
	tools := Tools.New()
	kubeCtlCmd := exec.Command("sudo", "/usr/bin/kubectl", "exec", "anc-ovn-0", "ovn-nbctl", "list", "Logical_Router_Port")
...
func listLogicalRoutersRoutes(ovnPod string, chassisDict Chassis.ChassisDict) {
	...
	for _, router := range lRouters {
		getRouteCommand := exec.Command("sudo", "/usr/bin/kubectl", "exec", ovnPod, "ovn-nbctl", "lr-route-list", router.name[0])
...
func showLogicalRouterRoutes(routerName string, ovnPod string) {
	outputData := [][]string{}
	tools := Tools.New()
	getRouteCommand := exec.Command("sudo", "/usr/bin/kubectl", "exec", ovnPod, "ovn-nbctl", "lr-route-list", routerName)
	out, err := getRouteCommand.Output()
	if err != nil {
		log.Fatal(err)
	}
...
func listLogicalRoutersNat(ovnPod string, chassisDict Chassis.ChassisDict) {
	...
	for _, router := range lRouters {
		if len(router.name) > 0 {
			getRouteCommand := exec.Command("sudo", "/usr/bin/kubectl", "exec", ovnPod, "ovn-nbctl", "lr-nat-list", router.name[0])
			out, err := getRouteCommand.Output()
			if err != nil {
				log.Fatal(err)
			}
...
func showLogicalRouterNat(routerName string, ovnPod string) {
	...
	data := []string{"", "", "", ""}
	tools := Tools.New()
	getRouteCommand := exec.Command("sudo", "/usr/bin/kubectl", "exec", ovnPod, "ovn-nbctl", "lr-nat-list", routerName)
	out, err := getRouteCommand.Output()
	if err != nil {
		log.Fatal(err)
	}
